...
class Spectrum(TimeBin):
    def __init__(self, system, sigma_x, sigma_xdag, *pulses, dt=0.1, tend=800, dt_small=0.1, n_big=10, simple_exp=True, gaussian_t=None, gaussian_dt=False, verbose=False, workers=15, options={}, dm=False, sigma_x_mat=None, sigma_xdag_mat=None, t_mem=10, dt_big=None, add_tend=True) -> None:
        self.dm = dm
        self.tend = tend
        self.tl_map = None
        self.tl_dms = None
    ...
    def get_time_dependent_spectrum(self, tend, omega_min=-5, omega_max=5, domega=0.1):
        """
        To calculate the time-dependend spectrum S(omega, t) = Re(int_{0}^(t) dt' int_{0}^{t-t'}d tau G1(t',tau) exp(-i omega tau))
        we need to calculate G1(t',tau) on a regular timegrid which is the same for t and tau
...
f1,s1 = a.get_spectrum(dm=True, final_spectrum_only=True)
plt.clf()
plt.xlim(-4, 4)
spectral_cut = np.where((f1 >= -4) & (f1 <= 4))
plt.plot(f1[spectral_cut], np.log(np.abs(s1[spectral_cut]))+0.001, label="Spectrum (tl)")
plt.xlabel("Frequency (a.u.)")
plt.ylabel("Spectrum")
plt.legend()
plt.savefig("spectrum_tl.png")
# # f2,s2 = a.get_spectrum(dm=False, final_spectrum_only=True)

# # np.savez("spectrum_data.npz", f1=f1, s_tl=s1, f2=f2, s_explicit=s2)
...
# spectral_cut = np.where((f1 >= -4) & (f1 <= 4))
# plt.plot(f1[spectral_cut], np.log(np.abs(s1[spectral_cut]))+0.001, label="Spectrum (tl)")
# plt.plot(f2[spectral_cut], np.log(np.abs(s2[spectral_cut]))+0.001, label="Spectrum", dashes=[4,2])
# plt.xlabel("Frequency (a.u.)")
# plt.ylabel("Spectrum")
# plt.legend()
# plt.savefig("spectrum.png")

# a.G2_frequency(0, 0, 50, 100, regular_t=False)
