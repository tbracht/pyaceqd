...
def sixls_linear(t_start, t_end, *pulses, dt=0.5, delta_b=4, gamma_e=1/100, gamma_b=None, gamma_d=0, bx=0, bz=0, phonons=False, ae=3.0, temperature=4, verbose=False, lindblad=False, temp_dir='/mnt/temp_data/', pt_file=None, suffix="", \
         ...
         firstonly=False,calibration_file=None, print_hamiltonian=False):
    ...
    if bx != 0:
        # coupels X and Y to S and F, respectively
    ...
    result = system_ace_stream(t_start, t_end, *pulses, dt=dt, phonons=phonons, t_mem=t_mem, ae=ae, temperature=temperature, verbose=verbose, temp_dir=temp_dir, pt_file=pt_file, suffix=suffix, \
                  ...
                  dressedstates=dressedstates, rf_op=rf_op, rf_file=rf_file, firstonly=firstonly, print_H=print_hamiltonian)
    if output_dm:
        return compose_dm(result, dim=6)
    return result

def sixls_linear_dressed_states(t_start, t_end, *pulses, plot=True, t_lim=None, e_lim=None, filename="sixls_linear_dressed", firstonly=False, visible_states=None, print_states=None, return_eigenvectors=False, **options):
    colors = ["#0000cf", "#45b0ee", "#ff0022", "#9966cc", "#009e00", "#ffde39"]
    dim = 6
    return dressed_states(sixls_linear, dim, t_start, t_end, *pulses, filename=filename, plot=plot, t_lim=t_lim, e_lim=e_lim, firstonly=firstonly, colors=colors, visible_states=visible_states, return_eigenvectors=return_eigenvectors, print_states=print_states, **options)
