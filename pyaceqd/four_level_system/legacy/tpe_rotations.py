from pyaceqd.four_level_system.legacy.linear import biexciton
import numpy as np
...
class TPERotations():
    ...
    def generate_pt(self):
        ...
        biexciton(0,8*self.tau,p1,delta_xy=self.delta_xy,delta_b=self.delta_b,dt=self.dt ,t_mem=self.t_mem, lindblad=False, phonons=True, ae=self.ae,temperature=self.temperature,prepare_only=False,  pt_file=self.pt_name)
        # , factor_ah=self.ah_ratiophonon_factor=self.phonon_factor,, J_file=self.J_from_file
    ...
    def calc_timedynamics(self, tau, area, path="", save=False, plot_pulse=False, detuning=0, tend=None, plot=False, plotlims=None, lindblad=True, carve_pulse=False, pulse_args={"width_t": 4, "central_f": 0}, filter_width=0.14):
        ...
        if carve_pulse:
            ...
            t,g,x,y,b = biexciton(0,tend,p1,lindblad=lindblad, pulse_file=pulse_file, **self.options)
            if plot_pulse:
            ...
            t,g,x,y,b = biexciton(0,tend,p1,lindblad=lindblad, **self.options)
        if plot:
            plt.clf()
            plt.plot(t.real,np.real(x),label="x")
            plt.plot(t.real,np.real(y),label="y")
            plt.plot(t.real,np.real(b),label="b")
            ...
            plt.clf()
        if save:
            export_csv(path+"timedynamics_{:.2f}ps_{:.2f}pi.csv".format(tau,area), t.real, x.real, y.real, b.real)
        return t.real,g,x,y,b
    
    def get_rabi_rotations(self,detuning=0, integrate=True, plot=False, delete_pt=True, path="", workers=15, carve_pulse=False, pulse_args={"width_t": 4, "central_f": 0}, filter_width=0.14, exp_data=None, plot_dynamic=False):
        """
        returns the tpe-rabi rotations for the given parameters
        ...
        if carve_pulse:
            ...
            filename += "carve_{:.1f}ps_{:.1f}nm_".format(pulse_args["width_t"],filter_width)
        if self.phonons:
        ...
        def plot_data(areas, x,y,b, exp_data=None):
            # plotting of the results, is used with data that is already present or calculated
            plt.clf()
            plt.plot(areas, x, label="x")
            plt.plot(areas, y, label="y")
            plt.plot(areas, b, label="b")
        ...
        if os.path.exists(filename+".csv"):
            ...
            b = data[:,3]
            if plot:
                plot_data(areas, x,y,b, exp_data=exp_data)
            return areas, x,y,b
        
        # first generate process tensors
        if self.phonons and not os.path.exists(self.pt_name+"_initial"):
            self.generate_pt()
        results = np.zeros([3,len(self.areas)])
        ...
        with tqdm.tqdm(total=len(self.areas),leave=None) as pbar:
            with ThreadPoolExecutor(max_workers=workers) as executor:
                for i in range(len(self.areas)):
                    ...
                    if carve_pulse:
                        pulse = pg.PulseGenerator(0,np.round(10/self.gamma_e),0.02)
                        pulse.add_gaussian_time(t0=100, sig_or_fwhm='fwhm', field_or_intesity='int',area_time=self.areas[i],**pulse_args)
                        pulse.add_filter_double_erf(central_f=0,width_f=filter_width,rise_f=0.01)
                        pulse.apply_frequency_filter()
                    ...
                    if integrate:
                        ...
                        _e = executor.submit(biexciton,0,np.round(10/self.gamma_e)+100,p1,lindblad=True, suffix=i, pulse_file_x=pulse_file, **self.options)
                    else: 
                        # use last time step as result otherwise
                        _e = executor.submit(biexciton,0,8*self.tau,p1,lindblad=False, suffix=i, pulse_file_x=pulse_file, **self.options)
        ...
        for i in range(len(self.areas)):
            ...
            if integrate:
                results[0,i] = self.gamma_e*np.trapezoid(np.real(x),np.real(t))
                results[1,i] = self.gamma_e*np.trapezoid(np.real(y),np.real(t))
                results[2,i] = 2*self.gamma_e*np.trapezoid(np.real(b),np.real(t))  # factor 2 because of the two excitons
            else:
                results[0,i] = x[-1].real
                results[1,i] = y[-1].real
                results[2,i] = b[-1].real
        #if self.phonons:
        export_csv(filename+".csv", self.areas, *results.real)
        #else:
        #    export_csv(path+"rabi.csv", self.areas, results)
        if plot:
            plot_data(self.areas, *results.real, exp_data=exp_data)
        if delete_pt:
