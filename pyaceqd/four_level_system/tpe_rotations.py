...
class TPERotations():
    ...
    def get_rabi_rotations(self,detuning=0, integrate=True, plot=False, delete_pt=True, path="", workers=15, carve_pulse=False, pulse_args={"width_t": 4, "central_f": 0}, filter_width=0.14, exp_data=None, plot_dynamic=False):
        ...
        for i in range(len(self.areas)):
            ...
            if integrate:
                results[0,i] = self.gamma_e*np.trapz(np.real(x),np.real(t))
                results[1,i] = self.gamma_e*np.trapz(np.real(y),np.real(t))
                results[2,i] = 2*self.gamma_e*np.trapz(np.real(b),np.real(t))  # factor 2 because of the two excitons
            else:
